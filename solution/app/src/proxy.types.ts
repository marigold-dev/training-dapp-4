...
type Methods = {
  call: (entrypointName: string, payload: bytes) => Promise<void>;
  upgrade: (
    _0: Array<{
      entrypoint?: {
        addr: address;
...
type MethodsObject = {
  call: (params: { entrypointName: string; payload: bytes }) => Promise<void>;
  upgrade: (params: {
    0: Array<{
      entrypoint?: {
        addr: address;
...
type contractTypes = {
  ...
  storage: Storage;
  code: { __type: "ProxyCode"; protocol: string; code: object[] };
};
export type ProxyContractType =
  ContractAbstractionFromContractType<contractTypes>;
export type ProxyWalletType =
